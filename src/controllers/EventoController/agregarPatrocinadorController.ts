...
const agregarPatrocinador = async (req: Request, res: Response) => {
    try {
        ...
        const patrocinadorDto = new PatrocinadorDto(nombre, logo_url || '');
        const resultado = await EventoService.agregarPatrocinador(Number(id), patrocinadorDto);
        if (!resultado.success) {
            return res.status(400).json({ status: 'Error', message: resultado.message });
        }
        res.status(201).json({ status: 'Éxito', patrocinador: resultado.patrocinador });
    } catch (error: any) {
