...
const addFavorito = async (req: CustomRequest, res: Response) => {
    try {
        ...
        const resultado = await FavoritoService.addFavorito(
            Number(id_cliente),
        ...
        console.error('Error en el controlador al agregar favorito:', error);
        res.status(500).json({ 
            status: 'Error',
